...
import com.example.foodrunners.database.RestaurantEntity
...
class AllRestaurantsAdapter(private var restaurants: ArrayList<Restaurants>, val context: Context) :
    ...
    override fun onBindViewHolder(p0: AllRestaurantsViewHolder, p1: Int) {
        ...
        val costForTwo = "Rs.${resObject.costForTwo.toString()}/person"
        p0.cost.text = costForTwo
        Picasso.get().load(resObject.imageUrl).error(R.drawable.cutlery).into(p0.resThumbnail)


        val listOfFavourites = GetAllFavAsyncTask(context).execute().get()

        if (listOfFavourites.isNotEmpty() && listOfFavourites.contains(resObject.id.toString())) {
            p0.favImage.setImageResource(R.drawable.ic_action_checked_fav)
        } else {
            p0.favImage.setImageResource(R.drawable.ic_action_unchecked_fav)
        }
